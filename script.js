...
function parseData() {
    ...
    const tableData = sections.map((section) => {
        ...
        const dexterity = section.match(/Dexterity: (\d+,?\d*)/)[1].replace(/,/g, "");
        ...
        return [name + "[" + id + "]", strength, speed, dexterity, defense, total, dexterity2, defense2];
    });
    ...
    const tableHtml =
      ...
      tableData.map((rowData) => {
        return "<tr>" +
          rowData.map((data, i) => {
            return (i === 0) ? "<td>" + data + "</td>" : "<td>" + numberWithCommas(data) + "</td>";
          }).join("") +
          "</tr>";
      }).join("") +
      "</tbody>" +
...
function sortTable() {
    ...
    rowData.forEach(rowData => {
        ...
        row.append(strengthCell, defenseCell, speedCell, dexterityCell, totalCell, dex2Cell, def2Cell);
